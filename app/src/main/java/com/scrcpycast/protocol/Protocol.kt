package com.scrcpycast.protocol

import java.io.InputStream
import java.io.OutputStream
import java.nio.ByteBuffer

import com.scrcpycast.server.LeboCastManager

object Protocol {
    const val MAGIC = 0x53435250
    const val DEFAULT_PORT = 37280

    val LELINK_PORTS = setOf(0xcc14, 7000, 6000)

    fun isLelinkPort(port: Int): Boolean = port in LELINK_PORTS

    @JvmStatic
    var leboCastManager: LeboCastManager? = null

    @JvmStatic
    fun stopLeboCast() {
        leboCastManager?.stop()
        leboCastManager = null
    }
    ...
    fun writeCodecConfig(out: OutputStream, codec: Int, width: Int, height: Int, data: ByteArray) {
        ...
        // Forward codec config to LeboCastManager as RTP SPS/PPS packets
        leboCastManager?.let { manager ->
            manager.sendCodecConfig(data)
        }
    }
    ...
    fun writeFrame(out: OutputStream, codec: Int, width: Int, height: Int, pts: Long, data: ByteArray) {
        ...
        // Forward frames to LeboCastManager for lelink protocol
        leboCastManager?.let { manager ->
            manager.sendEncodedFrame(data, 0, data.size, pts)
        }
    }
